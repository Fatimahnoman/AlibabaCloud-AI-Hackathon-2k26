...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (typeof data.verificationStatus !== 'undefined' && (typeof data.verificationStatus !== 'string' || data.verificationStatus.trim().length === 0)) {
      return errorResponse('verificationStatus must be a non-empty string', 'VALIDATION_ERROR', 400);
    }

    const updated = await admin.updateSource(id, data, reason);
    if (!updated) {
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const deleted = await admin.deleteSource(id, reason);
    if (!deleted) {
      return errorResponse('Source not found', 'NOT_FOUND', 404);
    }
