...
function getSecrets() {
  ...
  return {
    jwtSecret: process.env.JWT_SECRET || 'dev-only-jwt-secret-do-not-use-in-prod',
...
export function generateAccessToken(payload: JWTPayload): string {
  ...
  return jwt.sign(payload, jwtSecret, { expiresIn: JWT_EXPIRES_IN, algorithm: 'HS256' } as jwt.SignOptions);
}
...
export function generateRefreshToken(userId: string): { token: string; expiresAt: Date } {
  ...
  const token = jwt.sign({ userId }, refreshSecret, { expiresIn: JWT_REFRESH_EXPIRES_IN, algorithm: 'HS256' } as jwt.SignOptions);
...
export function verifyRefreshToken(token: string): { userId: string } | null {
  try {
    const { refreshSecret } = getSecrets();
    const decoded = jwt.verify(token, refreshSecret, { algorithms: ['HS256'] }) as { userId: string };
    return decoded;
  } catch {
    return null;
  }
